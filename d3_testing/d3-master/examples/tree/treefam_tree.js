...
// <linearGradient style="-webkit-tap-highlight-color: rgba(0, 0, 0, 0);" 
// id="9790-_fff-_7a1e74:50-_7a1e74:70-_fff" x1="0" y1="1" x2="6.123031769111886e-17" y2="0" 
// gradientTransform="matrix(1,0,0,1,0,0)">
// <stop style="-webkit-tap-highlight-color: rgba(0, 0, 0, 0);" offset="0%" stop-color="#ffffff"></stop>
// <stop style="-webkit-tap-highlight-color: rgba(0, 0, 0, 0);" offset="50%" stop-color="#7a1e74"></stop>
// <stop style="-webkit-tap-highlight-color: rgba(0, 0, 0, 0);" offset="70%" stop-color="#7a1e74"></stop>
// <stop style="-webkit-tap-highlight-color: rgba(0, 0, 0, 0);" offset="100%" stop-color="#ffffff"></stop>
// </linearGradient>
	...
	var tree;
	var domainOnlyScale = d3.scale.linear().domain([0,5]);

function draw_d3_tree(no_genes, json_tree, images_dir){
	console.log("no_genes is: "+no_genes);
	console.log("tree  is: "+json_tree);
 	h = no_genes * 10;
	image_path = images_dir;
	var id_label;
	 w = "700", h = h, i = 0, duration = 1000, root;
...
// DOMAIN        
       var all_domains =  domains.enter().append("rect")
					   .attr("y", function(d){
					   			return 180 + domainOnlyScale(d.domain_start);
					   	})
					   .attr("x", 193)
                       .attr("class", "domain")
					   .attr("rx", 5)
					   ...
					   .attr("height", function(d){
							   var length = domainOnlyScale(d.domain_stop - d.domain_start);
...
// 	      });
					...
					//dom_labels.append("title").text(d.name); 
			 dom_labels.enter().append("svg:text")
 						.attr("y", function(d){return -7;})
 						.attr("x", function(d,i){return leaf_group_x-10 + domainOnlyScale(d.domain_start);})
 						.style("fill", "black")
 						.attr("font-size" ,"5px;")
...
// External functions
	...
	// IDS
	function show_ids(d) {
		 d3.select("#tree svg").selectAll("text").text(function(d) { 
			return  d.name; 
		});
	}
